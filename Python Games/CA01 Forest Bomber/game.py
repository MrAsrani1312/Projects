...
level_cleared = False
plane_front = 0
plane_explosion_sound_played = False
...
# Set Up different formats for each level
forest_1 = ['T', '-', 'T', '-', '-', '-', 'T', '-', '-', '-', '-', 'T']
...
while True:
	...
	for event in pygame.event.get():
		...
		key_presssed = pygame.key.get_pressed()
		if key_presssed[pygame.K_SPACE]:
			if bomb_droppped is False and level_cleared is False and plane_exploded is False:
				bomb_droppped = True
				bomb.x = plane.x + 15
				bomb.y = plane.y + 10

		#Return Key Pressed at end of game/level
		elif key_presssed[pygame.K_RETURN]:

			# Plane has exploded or all levels cleared -> back to start
			...
			# Level Cleared -> go up 1 level and load new forest 
	...
	# Display Scoreboard - Score, Lvl, High Score
	...
	hi_text = 'High Score: ' + str(hi_score)
	...
	if plane_exploded is True or level_cleared is True:
		...
		if plane_exploded is True:
			...
			if plane_explosion_sound_played is False:
				explosion_sound.play()
				plane_explosion_sound_played = True
		...
		# Display messagebox to sit text over
		...
		# Display 2 lines of text, centered
